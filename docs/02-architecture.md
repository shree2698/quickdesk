...
- **Generative RAG AI**:
  1. **Indexing**: Internal markdown documents are added in two ways: via initial seeding from the `backend/knowledge-base/` directory during setup, or dynamically uploaded via the admin UI to `backend/uploads/knowledge-base/`. Both sources are loaded, split into chunks of ~1000 characters with 200-character overlaps, and embedded using Gemini's `gemini-embedding-001` model.
